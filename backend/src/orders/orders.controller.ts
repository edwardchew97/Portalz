...
export class OrdersController extends BaseController {
	...
	@Post('/create')
	async create(@Body() createOrderDto: CreateOrderInputDto): Promise<CommonResponse<Order>> {
	...
	@Get('/rate/:crypto')
	async getRate(@Param() params: GetRateDto): Promise<CommonResponse<CreateOrderOutputDto>> {
		...
		return this.response(200, 'Retrieved rate successfully.', {
			crypto_rate, near_rate, swap_rate: crypto_rate / near_rate
		})
	}
	...
	@Post('/verify')
	async verifyPayments(@Body() verifyOrderDto: VerifyOrderDto): Promise<CommonResponse<Order>> {
		...
		if (order) {
			...
			let amountReceived: any = trx.meta.postBalances[1] - trx.meta.preBalances[1];
			...
			let resultOrder = await this.ordersService.repository.save({ id: order.id, from_amount: amountReceived, to_amount: amountToSend.toString(), status: STATUS.RECEIVED, transaction_hash: result.transaction.hash })

			return this.response(200, 'Order verified successfully and near is sent', resultOrder)
			// } else {
